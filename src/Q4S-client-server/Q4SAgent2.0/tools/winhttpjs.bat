...
function request(url) {
    ...
    if (saveTo !== "" && saveTo.toLowerCase() !== "con") {
        ...
        try {
			...
			writeFile(saveTo, WinHTTPObj.GetAllResponseHeaders());
            addTextToFile(saveTo, "Content-Type: application/json");
...
function addTextToFile(fileName, data) {
    AdoDBObj.Type = 2;
    AdoDBObj.CharSet = "iso-8859-1";
    AdoDBObj.Open();    
    AdoDBObj.LoadFromFile(fileName);
    AdoDBObj.ReadFile
    AdoDBObj.Position = AdoDBObj.Size
    AdoDBObj.WriteText(data);
    AdoDBObj.SaveToFile(fileName, 2);
    AdoDBObj.Close();
}
