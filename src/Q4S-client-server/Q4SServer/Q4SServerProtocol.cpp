...
bool Q4SServerProtocol::measureStage0(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult &upResults, unsigned long pingsToSend)
{
    ...
    if (ok)
    {
		...
		// Show
		showMeasure(false, false, false, upMeasurements.latency, results.values.latency, upMeasurements.jitter, results.values.jitter, 0.f, 0.f, 0.f, 0.f);
    }
...
bool Q4SServerProtocol::measureContinuity(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult &upResults, unsigned long pingsToSend)
{
    ...
    if (ok)
    {
		...
		showMeasure(
			false, true, false, 
			upMeasurements.latency, results.values.latency, 
			upMeasurements.jitter, results.values.jitter, 
			upMeasurements.packetLoss, results.values.packetLoss, 0.f, 0.f);
	}
...
bool Q4SServerProtocol::measureStage1(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult upResults)
{
	...
	if (ok)
    {
		...
		showMeasure(
			...
			0.f, 0.f,
			0.f, 0.f,
			upMeasurements.packetLoss, results.values.packetLoss, 
			upMeasurements.bandwidth, results.values.bandwidth);
	}
